...
class ServerSocketManager(
    ...
    private data class ClientSocket(
        val socket: Socket,
    ...
    private val clientsMap = mutableMapOf<String, ClientSocket>()
    ...
    private suspend fun startForChatMessaging() = withContext(Dispatchers.IO) {
        try {
            ...
            while (isActive && !ss.isClosed) {
                try {
                    ...
                    // Handle client in a separate coroutine to avoid blocking the accept loop
                    launch {
                        handleChatClient(socket)
                    }
                } catch (e: Exception) {
    ...
    private suspend fun handleChatClient(socket: Socket) {
        ...
        try {
            ...
            // Read user info as first object (Handshake)
            val user = withContext(Dispatchers.IO) { input.readObject() }
            if (user is User) {
                clientId = user.id
                mapMutex.withLock {
                ...
                onClientConnected?.invoke(
                    Client(
                        ...
                        socket.inetAddress.hostAddress == "192.168.49.1",
                        user
                    )
                )
                ...
                while (currentCoroutineContext().isActive) {
                    try {
                        val obj = withContext(Dispatchers.IO) { input.readObject() } ?: break
                ...
                Log.e("ServerSocketManager", "$socketType:: Unexpected first object from client")
            }
        } catch (e: Exception) {
            Log.e("ServerSocketManager", "$socketType:: Error handling client: ${e.message}")
        } finally {
            clientId?.let { removeClient(it) } ?: run {
    ...
    private suspend fun startForFileTransfer() = withContext(Dispatchers.IO) {
        try {
            val ss = ServerSocket(port)
            serverSocket = ss
            Log.d("ServerSocketManager", "$socketType:: ServerSocket created on port $port")
    ...
    private suspend fun handleFileClient(socket: Socket) {
        try {
            val out = ObjectOutputStream(socket.getOutputStream())
            val input = ObjectInputStream(socket.getInputStream())
            ...
            Log.e("ServerSocketManager", "$socketType:: Error in file transfer handshake: ${e.message}")
            withContext(Dispatchers.IO) {
                try { socket.close() } catch (_: Exception) {}
            }
        }
    ...
    private suspend fun removeClient(clientId: String) {
        val client = mapMutex.withLock { clientsMap.remove(clientId) }
        if (client != null) {
    ...
    suspend fun sendMessageToClient(message: ActionMessage, clientId: String) {
        val client = mapMutex.withLock { clientsMap[clientId] }
        if (client != null) {
            try {
                withContext(Dispatchers.IO) {
    ...
    suspend fun readBytesFromClient(clientId: String, buffer: ByteArray, toRead: Int): Int {
        val client = mapMutex.withLock { clientsMap[clientId] }
        var bytesRead = -1
        if (client != null) {
            try {
                bytesRead = withContext(Dispatchers.IO) {
                    client.inputStream.read(buffer, 0, toRead)
                }
            } catch (e: IOException) {
    ...
    suspend fun readNextMessageFromClient(clientId: String): ActionMessage? {
        ...
        if (client != null) {
            ...
            Log.e("ServerSocketManager", "$socketType:: Client $clientId not found for reading message")
        }
        return message
    }
    ...
    fun onReceiveMessage(message: Any, clientId: String) {
        ...
        if (message is ActionMessage) {
            ...
            Log.e("ServerSocketManager", "$socketType:: Received invalid message type")
        }
    }

    fun stop() {
        serverScope.launch {
            val snapshot = mapMutex.withLock { clientsMap.keys.toList() }
